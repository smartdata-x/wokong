...
object HbaseSpark {
  ...
  //得到hbase里的数据并作为RDD
  def obtainHbaseRdd(tablename: String): Unit = {
    ...
    //排序+topn
    val array = hbaseRdd.flatMap(change(_)).map((_, 1)).reduceByKey(_ + _).map(x => (x._2, x._1)).sortByKey(false).top(100)
...
//          println(x._2)
        ...
        val fileimp = new File("/home/shj/industry_words1.words")
        println("fileimp:"+fileimp)
        Source.fromFile(fileimp, "UTF-8").getLines().foreach(keyValueimp => {
...
val filestockk=new File("/home/shj/stock.txt")
  ...
  /**
    * 分词的方法
    **/
  def change(tuple: (ImmutableBytesWritable, Result)): mutable.MutableList[String] = {

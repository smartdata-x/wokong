...
object TimeHandle {
  ...
  def getMonth(cal: Calendar): Int = cal.get(Calendar.MONTH) + 1
  ...
  def getDay(cal: Calendar): Int = cal.get(Calendar.DAY_OF_MONTH)
  ...
  def getMinuteTimeStamp(timeStamp: Long): Long = {
  ...
  def timeTamp(dateStr: String, formatStr: String): Long = {
    new SimpleDateFormat(formatStr).parse(dateStr).getTime
  }
